...
# Parameters.
source("input/SWAN/parsForWomen30years_best_case_model.R") 
...
personData <- data.frame(startAge = 70)
systemTimeOutput <- system.time(result <- processPersonsAboveDiagnosisThreshold(v0, v1other, v2, threshold=2.5, personData = personData))
...
if (doPSA) {
  ...
  systemTimeOutput <- system.time(
    psaResultNew <-psaAboveDiagnosisThreshold(v0,v1other,v1distributions,threshold=2.5, personData = personData))
