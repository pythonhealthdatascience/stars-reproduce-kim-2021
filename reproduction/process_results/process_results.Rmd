...
```{r}
make_sup_tab <- function(df, colname){
  #' Create components of supplementary table
  #' 
  #' Provided as function as there was a lot of repetition to create each of
  #' these elements
  #' @param df Dataframe with model results
  #' @param colname Name of final column in supplementary table
  ...
  # Combine scenario 0 (period=0) with the results from period 0.25+
  df_comb <- rbind(surv_s0 %>% mutate("period" = 0) %>% select(any_of(cols)),
                   df %>% select(any_of(cols)))

  # Calculate scaled excess deaths and emergencies
  sup_tab <- make_tab(df_comb, scale_to=15376, groupvar="period") %>%
    rename(!!colname := excess_dead_emer) %>%
...
```{r}
# Create each of the columns of the table, adding the full column names
suptab1_s1 <- make_sup_tab(surv_s1, "Scan suspension only (S1)")
suptab1_s21 <- make_sup_tab(surv_s4a, "+10% dropout/ annum for 1y (S2.1)")
suptab1_s22 <- make_sup_tab(surv_s4b, "+10% dropout/ annum for 2y (S2.2)")
suptab1_s23 <- make_sup_tab(surv_s4c, "+7cm threshold for 2y (S3)")
# Combine into single table
suptab1 <- mutate(suptab1_s1, suptab1_s21, suptab1_s22, suptab1_s23) %>%
  # Only keep 6 months +
  filter(period >= 0.5) %>%
  # Relabel
  mutate(period = paste0(period*12, "m")) %>%
...
# Save to csv
write.csv(suptab1, paths$suptab1, row.names=FALSE)
