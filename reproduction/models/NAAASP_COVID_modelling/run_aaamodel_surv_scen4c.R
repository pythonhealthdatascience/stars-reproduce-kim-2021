...
## Tricks because modelling all those under surveillance - ie have already attending initial scan prior to model run
## Need to trick model by setting cost of initial invitation to 0
## Also need to set attendance probability (of initial invitation) to 1
...
period<-0.25
thresh<-7
scen4csummary<-data.frame(n,dropoutrate,dropoutperiod,period,thresh,inv,scr,reinv,nonatt,monitor,dropout,oppdet,consult,elecevar,elecopen,rupt,emerevar,emeropen,reintelecevar,reintemerevar,reintemeropen,aaadead,nonaaadead)
...
scen4csummary<-rbind(scen4csummary,temp)
...
write.csv(scen4csummary, "output/output_surv_scen4c.csv", row.names=FALSE)
